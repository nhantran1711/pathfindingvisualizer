// Using stack to explore Tile

import { getUntraversedNeighbors } from "../../utls/getUntraversedNeighbors";
import { checkDFSStack, isEqual } from "../../utls/helpers";
...
export const dfs = (grid: GridType, startTile: TileType, endTile: TileType) => {
    ...
    while (unTraversedTiles.length > 0) {
        const current = unTraversedTiles.pop();
        if (current) {
            ...
            for (let i = 0; i < neighbors.length; i ++ ) {
                if (!checkDFSStack(neighbors[i], unTraversedTiles)) {
